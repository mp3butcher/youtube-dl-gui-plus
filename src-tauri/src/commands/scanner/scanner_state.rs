...
use crate::runners::mitmproxy_runner::MitmproxyChild;
...
/// Manages the TCP scanner connection state
#[derive(Clone)]
pub struct TcpScannerState {
    pub connection: Arc<TokioMutex<Option<TcpStreamHandle>>>,
}

/// Handle to the TCP stream and associated task
pub struct TcpStreamHandle {
    // We store a flag to signal shutdown
    pub shutdown_flag: Arc<TokioMutex<bool>>,
    // Store the mitmweb process so we can kill it
    pub mitmweb_child: Option<Arc<TokioMutex<MitmproxyChild>>>,
}

impl TcpScannerState {
    pub fn new() -> Self {
        Self {
...
impl Default for TcpScannerState {
    fn default() -> Self {
        Self::new()
    }
}

pub const MITMPROXY_SCRIPT_ENSURE_STRING : &str = r#"
import asyncio
import base64
