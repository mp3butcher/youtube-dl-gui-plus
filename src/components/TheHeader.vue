<template>
  ...
  <div> <button type="button" @click="startStopScanner" class="btn btn-subtle" :class="text === 'ON' ? 'btn-success' : 'btn-warning'"  :title="t('layout.header.nav.networkscan')"> {{ text }}
...
const lastwdurl = ref('');
const lastwdheader: Ref<{ k: string; v: string }[]> = ref([]);
/* let reqlicHeaders='';
...
onMounted(async () => {
  ...
  unlistenScanner = await listen('tcp_scanner', (event: CustomEvent) => {
    const msg: string = event.payload.message;
    const headerFilter = settingsStore.settings.mitmproxy.headerFilter;
    // if (scannerIsOn)
    {
      let data: ParsedJsonData;
      try {
        data = JSON.parse(msg) as ParsedJsonData;
      } catch (e: unknown) {
        console.error(e);
      ...
      if (contentype.startsWith('video') && contentlength > 1500000) {
        toscandeeply = true;
        console.warn('[x] contentype video!!!!!!!!!!!' + data.url);
      }
      // Large Content-Range
      if (!contentype.startsWith('image') && (sizeok || contentlength > 1500000)) {
        toscandeeply = true;
      }
      const res = atob(data.response);
      ...
      if (res.length > 1) {
        ...
        if (res[0] == '#') { // HLS?
          ...
          if (typeof (wd[0]) != 'undefined') {
            const bestqkey = 0; // Have to find a way to get desired stream
      ...
      if (toscandeeply) {
        const argheaders: Record<string, string> = {};
        ...
        mediaStore.dispatchMediaInfoFetch(data.url, false, argheaders).catch((e) => {
          console.error('Failed to fetch media info for ' + data.url, e);
          toastStore.showToast('Failed to fetch media info for ' + data.url, { style: 'error' });
        });
      }
    }
  });
